...
# ラグの影響を考慮した特徴量の追加
for columns in ['HUFL', 'HULL', 'MUFL', 'MULL', 'LUFL', 'LULL', 'OT']:
    df[f'{columns}_lag1'] = df[columns].shift(1)
    df[f'{columns}_lag6'] = df[columns].shift(6)
    df[f'{columns}_lag12'] = df[columns].shift(12)
    df[f'{columns}_lag24'] = df[columns].shift(24)
    df[f'{columns}_lag48'] = df[columns].shift(48)
